...
def get_write_file(d, wdir): # for creating or using a sub-file for each language, where html-pages will be stored
    
    new_write_file = join(wdir, "html", d["lang"])
...
def get_html_file(d, wdir): # will be used to get the right html pages, based on the htmlpages-parameter of the config.yaml
    
    html_folder = join(wdir, "html", d["lang"], "*.html")
...
def main(lang, basedir, level, wdir, results):
    ...
    new_write_file, d = get_write_file(d, wdir) #, write_file
    html_folder, d = get_html_file(d, wdir)
